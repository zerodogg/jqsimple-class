...
of parent, and the constructor for parent will be called when constructing the
object. First the constructor of the child will be called, then the parent, and so
on all the way up the inheritance chain, however long that might be. All constructors
recieve all the parameters, therefore it is a good idea to use key-value hashes/objects
as the parameter, rather than named positional parameters, so that it is possible
to supply useful values to all constructors, if that is needed. A child will override
a parents methods.
...
Returns the version number of the jqsimple-class library that is in use.
...
(none at the moment, namespace .jClass. reserved for future use)

=head1 NOTES

- You do not need to provide a constructor if you don't want to, it can be
omitted completely if you wish.

=head1 EXAMPLES

=over

=item Building a simple class
...
=item Extending a simple class and removing the constructor
...
=item Adding methods to an existing class
